...
class Event extends Component {
  ...
  componentWillMount = async () => {
    ...
    for (let i=0;i<eventDomainsFiltered.length;i++){
      ...
      let dataDefinitions = []
      if (domain.data.dataDefinitions){
  ...
  render() {
    ...
    // console.log(event)
    const heading = `Info event ID: ${this.props.match.params.id}`;
    return (
      <div className="animated fadeIn">
        <PageTitle
            heading= {heading}
            eventId={this.props.match.params.id}
            subheading="This page shows detail information of event"
            icon="fa fa-info icon-gradient bg-mean-fruit"
            mode="view"
        />
        <ViewEvent mode="view" event={event}></ViewEvent>
      </div>
    )
  }
}
...
const mapStateToProps = state => ({
  allEvents: state.EventOptions.allEvents,
});
